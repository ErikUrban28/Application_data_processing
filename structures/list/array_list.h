...
namespace structures
{
	...
	inline Structure& ArrayList<T>::assign(Structure& other)
	{
		if (this != &other)
		{
			const ArrayList<T>& otherList = dynamic_cast<const ArrayList<T>&>(other);
	...
	{
		Utils::rangeCheckExcept(index, size_, "Invalid index in ArrayList!");
	...
	inline void ArrayList<T>::add(const T& data)
	{
		...
		array_->at(static_cast<int>(size_)) = data;
		++size_;
	}
